...
public class CustomersAddOrUpdate extends Activity {
    ...
    public void addOrUpdateCustomer(View view){
        ...
        if(cust_name.length() < 4){
        ...
        } else if(Customer.find(Customer.class,"cardnumber=?",card_number).size() > 0) {
            showDialog("Error", "There is already a customer with Card #" + card_number + ".", false);
        } else {
            ...
            birthDay.set(bdpicker.getYear(), bdpicker.getMonth(), bdpicker.getDayOfMonth());
            
            java.text.DateFormat f = new java.text.SimpleDateFormat("MM/dd/yyyy");
            String birth_date = f.format(birthDay.getTime());
            if(custToEdit == null) {
                //We are creating a customer
            	Customer c =null;
            	List<Customer> customerList = Customer.find(Customer.class,"cust_name=? and strftime('%m/%d/%Y',birthday/1000,'unixepoch') = ?",cust_name,birth_date);
